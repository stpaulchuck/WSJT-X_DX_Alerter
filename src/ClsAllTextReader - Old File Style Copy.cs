	...
	class ClsAllTextReader
	{
		...
		public string[] ReadAllText()
		{
			...
			for (int iIndexer = iStringsMax; iIndexer > 0; iIndexer--) // splitArray[0] is likely a partial string so avoid it
			{
				...
				{
					string[] splitArray2 = splitArray[iIndexer].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
					sTemp2 = splitArray2[1];
					if (sTemp2[0] != '-') sTemp2 = "+" + sTemp2;
					// call sign plus time sent out
					lstCallSigns.Add(sTemp + " " + splitArray[iIndexer].Substring(0, 4) + " " + sTemp2); // these we send to be checked
		...
		string ParseAllString(string source)
		{
			...
			if (splitArray2[0].Contains("_"))
			{
				return retVal; // it was me transmitting so skip
			}
			if (splitArray2[0].Contains("-")) // it's a frequency change or newly started session
			{
				return "ZZZZ " + source;
			}
			// check threshold of rx level
			if (!int.TryParse(splitArray2[1], out iTemp))
			{
			...
			if (splitArray2.Length < 7)
				return retVal; // not a full qualified entry
			if (splitArray2[5].Trim() == "CQ")
			{
				if (splitArray2[6].Trim() == "DX")
				{
					retVal = splitArray2[7].Trim();
				}
				else
				{
					retVal = splitArray2[6].Trim();
				}
			}
			else
				retVal = splitArray2[6];
